package io.ledgerwise.ipfsresizer.helper.icafe4j.image.compression.packbits;
 ...
 * <p>
 * Contains both unpack and packing method for the Macintosh Packbits
...
public class Packbits {
	 ...
	 * Description
	 * 
	 * In choosing a simple byte-oriented run-length compression scheme, we
	 * arbitrarily
	 * chose the Apple Macintosh PackBits scheme. It has a good worst case behavior
	 * (at most 1 extra byte for every 128 input bytes). For Macintosh users, the
	 * toolbox
	 * utilities PackBits and UnPackBits will do the work for you, but it is easy to
	 * imple-ment
	 * your own routines.
	 * 
	 ...
	 * replicate run
	 * except when preceded and followed by a literal run. In that case, it is best
	 * to merge
	 * the three runs into one literal run. Always encode 3-byte repeats as
	 * replicate runs.
	 ...
	 * # If a run is larger than 128 bytes, encode the remainder of the run as one
	 * or more
	 * additional replicate runs.
	 * When PackBits data is decompressed, the result should be interpreted as per
	 * com-pression
