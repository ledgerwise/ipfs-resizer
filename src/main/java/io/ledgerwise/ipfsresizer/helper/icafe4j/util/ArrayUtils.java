...
public class ArrayUtils {
	 ...
	 * <p>
	 * Note: if type parameter is not explicitly supplied, it will be inferred as
	 * the
	 ...
	 * @return index of the search key or index of the element which is closest to
	 *         but less than the search key or
	...
	public static int findEqualOrLess(int[] a, int fromIndex, int toIndex, int key) {
		...
		// -index - 1 is the insertion point if not found, so -index - 1 -1 is the less
		// position
		if (index < 0) {
			index = -index - 1 - 1;
		}
	...
	/**
	 * Packs all or part of the input byte array which uses "bits" bits to use all 8
	 * bits.
	 * 
	 ...
	 * bits.
	 * <p>
	 * We assume len is a multiplication of stride. The parameter stride controls
	 * the packing
	 * unit length and different units <b>DO NOT</b> share same byte. This happens
	 * when packing
	...
	// Convert byte array to long array, then to integer array discarding the higher
	// bits
	...
	/**
	 * Converts an input byte array to nBits data array using the smallest data type
	 * which
	 ...
	 * @param bigEndian the packing order of the bits. True if bigEndian, otherwise
	 *                  false.
	 * 
