...
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.png.ChunkType;
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.png.ColorType;
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.png.Filter;
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.png.PNGDescriptor;
 ...
 * <p>
 * Supports up to 8 bit depth sequential and Adam7 interlaced color and
 * grayscale images.
 * Alpha channel and single color transparency are also supported where
 * permitted by the
 * image color type.
 * 
...
public class PNGReader extends ImageReader {
	...
	private static void apply_defilter(InputStream bis, byte[] pixBytes, int height, int bytesPerPixel,
			int bytesPerScanLine) throws Exception {
	...
	/*
	 * Gamma correction is much more complicated when several color management
	 * chunks are present.
	 * The following two variables are use to partially fix the incorrect gamma
	 * correction applied
	 * in these situations.
	 */
	...
	private void correctGamma(int[] image, int width, int height) {
		...
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++, p_index++) {
				...
				image[p_index] = ((image[p_index] & 0xff000000) | ((red & 0xff) << 16) | ((green & 0xff) << 8)
						| (blue & 0xff));
			}
		}
	}
	...
	private short[] generate16BitGrayscaleInterlacedPixels(byte[] compr_data) throws Exception {
		...
		// Wrap an InflaterInputStream with a bufferedInputStream to speed up reading
		BufferedInputStream bis = new BufferedInputStream(new InflaterInputStream(new ByteArrayInputStream(compr_data)));
		// Decoding the image pass by pass. There are 7 passes for ADAM7 interlacing
		// method.
			...
			if (fullAlpha) {
				...
				for (int j = 0, s_index = 0, k = 0; j < block_height; j++) {
					for (int i = 0; i < block_width; i++, p_index += x_inc) {
						...
						if (spixels[s_index] == redMask && spixels[s_index + 1] == greenMask
								&& spixels[s_index + 2] == blueMask) {
	...
	private byte[] generate8BitRGBInterlacedPixels(byte[] compr_data, boolean fullAlpha) throws Exception {
		...
		if (fullAlpha || alpha != null)
			bpixels = new byte[width * height * 4];
		////////////////////////////////////////////////////////////////////////////////////////////////
		// Wrap an InflaterInputStream with a bufferedInputStream to speed up reading
		BufferedInputStream bis = new BufferedInputStream(new InflaterInputStream(new ByteArrayInputStream(compr_data)));
		// Decoding the image pass by pass. There are 7 passes for ADAM7 interlacing
			...
			if (fullAlpha) {
				...
				for (int j = 0, k = 0; j < block_height; j++) {
					for (int i = 0; i < block_width; i++, p_index += x_inc, k += 3) {
						...
						if ((pix_interlaced[k] == alpha[1]) && (pix_interlaced[k + 1] == alpha[3])
								&& (pix_interlaced[k + 2] == alpha[5]))
	...
	private void generateGrayscaleInterlacedPixels(byte[] pixels, byte[] pix_interlaced, int block_width,
			int block_height, int padding, int p_index, int x_start, int y_start, int x_inc, int y_inc) {
		//
	...
	private void generateIndexedInterlacedPixels(byte[] pixels, byte[] pix_interlaced, int block_width, int block_height,
			int padding, int p_index, int x_start, int y_start, int x_inc, int y_inc) {
		//
		int i = 0, safeEnd = block_width - padding;
		switch (bitsPerPixel) {
			case 8:
	...
	private byte[] process_grayscaleInterlacedImage(byte[] compr_data) throws Exception {
		...
		for (int pass = 1; pass < 8; pass++) {
			...
			apply_defilter(bis, pix_interlaced, block_height, bytesPerPixel, bytesPerScanLine);
			generateGrayscaleInterlacedPixels(pixels, pix_interlaced, block_width, block_height, padding,
					x_start + width * y_start, x_start, y_start, x_inc, y_inc);
		}
	...
	private BufferedImage process_IDAT(byte[] compr_data) throws Exception {
		...
		if (hasICCP)
			colorSpace = new ICC_ColorSpace(ICC_Profile.getInstance(icc_profile));
		...
		switch (ColorType.fromInt(color_format)) {
			case GRAY_SCALE:
				...
				if (bitsPerPixel == 16) {
					...
					if (alpha != null) { // Deal with single color transparency
						...
						cm = new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_GRAY), true, false,
					...
					cm = new IndexColorModel(bitsPerPixel, rgbColorPalette.length, rgbColorPalette, 0, true, -1,
			...
			case GRAY_SCALE_WITH_ALPHA:
				...
				if (bitsPerPixel == 16) {
					...
					if (hasGamma && renderingIntent == -1 && !hasICCP)
						correctGamma(bpixels, width, height);
					db = new DataBufferByte(bpixels, bpixels.length);
					...
					cm = new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB), true, false,
			...
			case TRUE_COLOR:
				if (bitsPerPixel == 16) {
					...
					if (hasICCP) {
						...
						cm = new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB), nBits, hasAlpha, false,
			...
			case TRUE_COLOR_WITH_ALPHA:
				if (bitsPerPixel == 16) {
					...
					if (hasICCP) {
						raster = IMGUtils.iccp2rgbRaster(raster, cm);
						cm = new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB), nBits, true, false, trans,
	...
	private byte[] process_IndexedInterlacedImage(byte[] compr_data) throws Exception {
		...
		for (int pass = 1; pass < 8; pass++) {
			...
			generateIndexedInterlacedPixels(pixels, pix_interlaced, block_width, block_height, padding,
					x_start + width * y_start, x_start, y_start, x_inc, y_inc);
		}
	...
	public BufferedImage read(InputStream is) throws Exception {
		 ...
		 */
		// long signature =
		// ((IOUtils.readIntMM(is)&0xffffffffffffL)<<32)|IOUtils.readIntMM(is);
		 ...
		 * field.
		 * The length counts only the data field, not itself, the chunk type code, or
		 * the CRC.
		 ...
		 * files,
		 * type codes are restricted to consist of upper-case and lower-case ASCII
		 * letters
		 * (A-Z and a-z, or 65-90 and 97-122 decimal). However, encoders and decoders
		 * must treat
		 * the codes as fixed binary values, not character strings. For example, it
		 * would not be
		 * correct to represent the type code IDAT by the EBCDIC equivalents of those
		 * letters.
		 ...
		 * the chunk,
		 * including the chunk type code and chunk data fields, but not including the
		 * length field.
		 * The CRC is always present, even for chunks containing no data. See CRC
		 * algorithm.
		 */
		...
		LOGGER.info("image interlace method: {} - {}", interlace_method,
				PNGDescriptor.getInterlaceTypeDescription(interlace_method));
	...
	private void read_PLTE(InputStream is, int data_len) throws Exception {
		...
		for (int i = 0; i < len; i++)
			rgbColorPalette[i] = ((0xff << 24) | ((rgb_table[table_indx++] & 0xff) << 16)
					| ((rgb_table[table_indx++] & 0xff) << 8) | (rgb_table[table_indx++] & 0xff));
		IOUtils.readUnsignedIntMM(is);// CRC
	}
	...
	private byte[] readICCProfile(InputStream is, int data_len) throws Exception {
		...
		InflaterInputStream ii = new InflaterInputStream(
				new ByteArrayInputStream(buf, profileName_len + 2, data_len - profileName_len - 2));
