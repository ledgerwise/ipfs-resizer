...
package io.ledgerwise.ipfsresizer.helper.icafe4j.image.meta.image;
	...
	public Iterator<MetadataEntry> iterator() {
		if (containsThumbnail()) { // We have thumbnail
			...
			while (mapEntries.hasNext()) {
				...
				e.addEntry(new MetadataEntry("Thumbnail width",
						((thumbnail.getWidth() < 0) ? " Unavailable" : "" + thumbnail.getWidth())));
				e.addEntry(new MetadataEntry("Thumbnail height",
						((thumbnail.getHeight() < 0) ? " Unavailable" : "" + thumbnail.getHeight())));
