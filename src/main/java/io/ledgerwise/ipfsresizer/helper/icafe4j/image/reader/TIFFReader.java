...
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.color.CMYKColorSpace;
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.color.Int32ComponentColorModel;
...
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.compression.ccitt.G31DDecoder;
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.compression.ccitt.G32DDecoder;
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.compression.deflate.DeflateDecoder;
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.compression.lzw.LZWTreeDecoder;
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.compression.packbits.Packbits;
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.tiff.ASCIIField;
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.tiff.ByteField;
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.tiff.DoubleField;
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.tiff.FieldType;
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.tiff.FloatField;
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.tiff.IFD;
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.tiff.LongField;
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.tiff.RationalField;
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.tiff.ShortField;
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.tiff.TIFFTweaker;
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.tiff.Tag;
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.tiff.TiffField;
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.tiff.TiffFieldEnum;
...
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.tiff.UndefinedField;
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.util.IMGUtils;
import io.ledgerwise.ipfsresizer.helper.icafe4j.io.FileCacheRandomAccessInputStream;
import io.ledgerwise.ipfsresizer.helper.icafe4j.io.IOUtils;
import io.ledgerwise.ipfsresizer.helper.icafe4j.io.RandomAccessInputStream;
import io.ledgerwise.ipfsresizer.helper.icafe4j.io.ReadStrategyII;
import io.ledgerwise.ipfsresizer.helper.icafe4j.io.ReadStrategyMM;
...
public class TIFFReader extends ImageReader {
	...
	private BufferedImage decodeStrippedTiff(IFD ifd) throws Exception {
		...
		TiffFieldEnum.PlanarConfiguration e_planaryConfiguration = TiffFieldEnum.PlanarConfiguration
				.fromValue(planaryConfiguration);
		...
		if (f_extraSamples != null)
			isAssociatedAlpha = (f_extraSamples.getDataAsLong()[0] == 1) ? true : false;
		...
		if (planaryConfiguration == 2) {
			...
			for (int k = 0; k < samplesPerPixel; k++) {
				for (int j = startOff; j < endOff; j++)
					count[k] += stripBytes[j];
				startOff += inc;
				endOff += inc;
			}
		}
		...
		switch (e_photoMetric) {
			case PALETTE_COLOR:
				...
				for (int i = 0, index = 0; i < colorMap.length / 3; i++) {
					rgbColorPalette[index++] = 0xff000000 | ((colorMap[i] & 0xff00) << 8)
							| ((colorMap[i + numOfColors] & 0xff00)) | ((colorMap[i + numOfColors2] & 0xff00) >> 8);
				}
				...
				ColorModel cm = new IndexColorModel(bitsPerSample, rgbColorPalette.length, rgbColorPalette, 0, false, -1,
			...
			case SEPARATED:
				...
				if (bitsPerSample == 16) {
					...
					if (planaryConfiguration == 2) {
						...
						raster = Raster.createBandedRaster(db, imageWidth, imageHeight, bytesPerScanLine * 8 / bitsPerSample,
								bankIndices, bandoff, null);
					} else {
						raster = Raster.createInterleavedRaster(db, imageWidth, imageHeight, imageWidth * numOfBands,
								numOfBands, bandoff, null);
					}
					if (profile != null) {
						raster = IMGUtils.iccp2rgbRaster(raster, cm);
						cm = new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB), nBits, transparent,
								isAssociatedAlpha, trans, raster.getTransferType());
					}
				} else {
					cm = new ComponentColorModel(colorSpace, nBits, transparent, isAssociatedAlpha, trans,
							DataBuffer.TYPE_BYTE);
					if (planaryConfiguration == 2) {
						...
						raster = Raster.createBandedRaster(db, imageWidth, imageHeight, bytesPerScanLine, bankIndices,
								bandoff, null);
					} else {
						raster = Raster.createInterleavedRaster(db, imageWidth, imageHeight, imageWidth * numOfBands,
								numOfBands, bandoff, null);
					...
					if (profile != null) {
						raster = IMGUtils.iccp2rgbRaster(raster, cm);
						cm = new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB), nBits, transparent,
								isAssociatedAlpha, trans, DataBuffer.TYPE_BYTE);
					}
				}
			...
			case YCbCr:
				 ...
				 * B = Cb * ( 2 - 2 * LumaBlue ) + Y
				 * Default values: lumaRed = 299/1000, lumaGreen = 587/1000 and lumaBlue =
				 * 114/1000
				 */
				...
				if (planaryConfiguration == 1) {
					...
					switch (compression) {
						case NONE:
							for (int i = 0; i < stripByteCounts.length; i++) {
								...
								offsetY = upsampling(offsetY, numOfDataUnit, bytesPerUnitY, samplingFactor, referenceBlackY,
										referenceWhiteY, referenceBlackCb,
										referenceWhiteCb, referenceBlackCr, referenceWhiteCr, codingRangeY, codingRangeCbCr,
										lumaRed, lumaGreen, lumaBlue, temp,
						...
						case LZW:
							for (int i = 0; i < stripByteCounts.length; i++) {
								...
								offsetY = upsampling(offsetY, numOfDataUnit, bytesPerUnitY, samplingFactor, referenceBlackY,
										referenceWhiteY, referenceBlackCb,
										referenceWhiteCb, referenceBlackCr, referenceWhiteCr, codingRangeY, codingRangeCbCr,
										lumaRed, lumaGreen, lumaBlue, temp2,
										pixels, expandedImageWidth, dataUnitsPerWidth);
							}
					...
					if (stripByteCounts.length == 1 && samplesPerPixel != 1)
						throw new RuntimeException(
								"stripByteCounts length 1 is not consistent with samplesPerPixel " + samplesPerPixel);
						...
						case NONE:
							...
							for (int i = 0; i < expandedImageHeight; i++) {
								for (int j = 0; j < expandedImageWidth; j++, yPos++, yOffset++, counter++) {
									...
									float fCb = (Cb - referenceBlackCb) * codingRangeCbCr
											/ (referenceWhiteCb - referenceBlackCb);
									float fCr = (Cr - referenceBlackCr) * codingRangeCbCr
				...
				nBits = new int[] { 8, 8, 8 };
				raster = Raster.createInterleavedRaster(db, expandedImageWidth, expandedImageHeight,
						expandedImageWidth * numOfBands, numOfBands, bandoff, null);
				cm = new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB), nBits, transparent,
						isAssociatedAlpha,
					...
					if (floatSample) { // Floating point sample data type
						if (bitsPerSample == 16 || bitsPerSample == 32) {
							...
							throw new UnsupportedOperationException(
									"Unsupported floating point sample bit depth: " + bitsPerSample);
						}
						cm = new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB), nBits, transparent,
								isAssociatedAlpha, trans, dataBufferType);
						// Create a TYPE_FLOAT sample model (specifying how the pixels are stored)
						SampleModel sampleModel = new BandedSampleModel(dataBufferType, imageWidth, imageHeight, imageWidth,
								bankIndices, bandoff);
						...
						if (bitsPerSample == 16) {
							...
							if (bitsPerSample % 8 == 0)
								bigEndian = true;
							for (int i = 0; i < samplesPerPixel; i++)
								ints[i] = (int[]) (ArrayUtils.toNBits(bitsPerSample, rgb[i], samplesPerPixel * imageWidth,
										bigEndian));
						...
						cm = new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB), nBits, transparent,
								isAssociatedAlpha, trans, dataBufferType);
						raster = Raster.createBandedRaster(db, imageWidth, imageHeight, imageWidth, bankIndices, bandoff,
								null);
					}
				} else {
					if (floatSample) {
						if (bitsPerSample >= 16 && bitsPerSample <= 32) {
							if (bitsPerSample % 8 == 0) {
								...
								cm = new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB), nBits, transparent,
										isAssociatedAlpha, trans, DataBuffer.TYPE_FLOAT);
								db = new DataBufferFloat(tempArray, tempArray.length);
								// Create a TYPE_FLOAT sample model (specifying how the pixels are stored)
								SampleModel sampleModel = new PixelInterleavedSampleModel(DataBuffer.TYPE_FLOAT, imageWidth,
										imageHeight, samplesPerPixel, imageWidth * samplesPerPixel, bandoff);
								raster = Raster.createWritableRaster(sampleModel, db, null);
							} else
							...
							cm = new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB), nBits, transparent,
									isAssociatedAlpha, trans, DataBuffer.TYPE_DOUBLE);
							db = new DataBufferDouble(tempArray, tempArray.length);
							// Create a TYPE_FLOAT sample model (specifying how the pixels are stored)
							SampleModel sampleModel = new PixelInterleavedSampleModel(DataBuffer.TYPE_DOUBLE, imageWidth,
									imageHeight, samplesPerPixel, imageWidth * samplesPerPixel, bandoff);
							raster = Raster.createWritableRaster(sampleModel, db, null);
						} else
							throw new UnsupportedOperationException("Unsupported bit depth: " + bitsPerSample);
						...
						if (bitsPerSample < 8) {
							...
							cm = new DirectColorModel(bitsPerSample * samplesPerPixel, redMask[bitsPerSample],
									greenMask[bitsPerSample], blueMask[bitsPerSample]);
							...
							Object tempArray = ArrayUtils.toNBits(bitsPerSample, pixels, samplesPerPixel * imageWidth,
									(bitsPerSample % 8 == 0) ? endian == IOUtils.BIG_ENDIAN : true);
							if (bitsPerSample <= 16) {
								cm = new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB), nBits, transparent,
										isAssociatedAlpha, trans, DataBuffer.TYPE_USHORT);
							} else if (bitsPerSample == 32) { // A workaround for Java's ComponentColorModel bug with 32 bit
																			// sample
								cm = new Int32ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB), transparent);
							} else
								cm = new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB), nBits, transparent,
										isAssociatedAlpha, trans, DataBuffer.TYPE_INT);
							raster = cm.createCompatibleWritableRaster(imageWidth, imageHeight);
							raster.setDataElements(0, 0, imageWidth, imageHeight, tempArray);
						}
					}
			...
			case WHITE_IS_ZERO:
				...
				switch (bitsPerSample) {
					case 1:
						rgbColorPalette = (e_photoMetric == PhotoMetric.BLACK_IS_ZERO) ? BLACK_WHITE_PALETTE
								: BLACK_WHITE_PALETTE_WHITE_IS_ZERO;
						break;
					case 2:
						rgbColorPalette = (e_photoMetric == PhotoMetric.BLACK_IS_ZERO) ? FOUR_COLOR_PALETTE
								: FOUR_COLOR_PALETTE_WHITE_IS_ZERO;
						break;
					case 4:
						rgbColorPalette = (e_photoMetric == PhotoMetric.BLACK_IS_ZERO) ? SIXTEEN_COLOR_PALETTE
								: SIXTEEN_COLOR_PALETTE_WHITE_IS_ZERO;
						break;
					case 8:
						rgbColorPalette = (e_photoMetric == PhotoMetric.BLACK_IS_ZERO) ? EIGHT_BIT_COLOR_PALETTE
								: EIGHT_BIT_COLOR_PALETTE_WHITE_IS_ZERO;
						break;
					default:
				}
				switch (compression) {
					...
					case CCITTFAX3:
						...
						if ((t4Options & GROUP3OPT_UNCOMPRESSED) == GROUP3OPT_UNCOMPRESSED) {
							...
							decoder = new G32DDecoder(imageWidth, rowsPerStrip, true); // 1D encoding, need to take care of
																											// fill bit
						}
						break;
					case LZW:
				...
				if (bitsPerSample <= 8) {
					if (predictor == 2 && planaryConfiguration == 1)
						pixels = applyDePredictor(samplesPerPixel, pixels, imageWidth, imageHeight);
					db = new DataBufferByte(pixels, pixels.length);
					cm = new IndexColorModel(bitsPerSample, rgbColorPalette.length, rgbColorPalette, 0, false, -1,
							DataBuffer.TYPE_BYTE);
					raster = Raster.createPackedRaster(db, imageWidth, imageHeight, bitsPerSample, null);
					if (samplesPerPixel == 2) { // Deal with alpha transparency
						if (e_photoMetric == PhotoMetric.WHITE_IS_ZERO)
							IMGUtils.invertBits(pixels, 2); // Unlike samplesPerPixel == 1 case, we have to invert bits here as
																		// we are using ComponentColorModel
						numOfBands = samplesPerPixel;
						int[] bandOffsets = { 0, 0, 0, 1 };
						transparent = true;
						trans = Transparency.TRANSLUCENT;
						cm = new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB), transparent,
								isAssociatedAlpha, trans, DataBuffer.TYPE_BYTE);
						raster = Raster.createInterleavedRaster(db, imageWidth, imageHeight, imageWidth * numOfBands,
								numOfBands, bandOffsets, null);
					}
				} else if (bitsPerSample <= 16) { // Assume bitsPerSample <= 16
					short[] tempArray = (short[]) ArrayUtils.toNBits(bitsPerSample, pixels, samplesPerPixel * imageWidth,
							(bitsPerSample % 8 == 0) ? endian == IOUtils.BIG_ENDIAN : true);
					if (predictor == 2 && planaryConfiguration == 1)
					...
					cm = new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_GRAY), transparent, isAssociatedAlpha,
							trans, DataBuffer.TYPE_USHORT);
					raster = cm.createCompatibleWritableRaster(imageWidth, imageHeight);
					raster.setDataElements(0, 0, imageWidth, imageHeight, tempArray);
				} else if (bitsPerSample == 64) { // We can't use ColorSpace.CS_GRAY for some reason. Use CS_sRGB instead
					double[] tempArray = ArrayUtils.toDoubleArray(pixels, endian == IOUtils.BIG_ENDIAN);
					cm = new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB), null, transparent,
							isAssociatedAlpha, trans, DataBuffer.TYPE_DOUBLE);
					db = new DataBufferDouble(tempArray, tempArray.length);
					SampleModel sampleModel = new PixelInterleavedSampleModel(DataBuffer.TYPE_DOUBLE, imageWidth,
							imageHeight, samplesPerPixel, imageWidth * samplesPerPixel, new int[] { 0, 0, 0 });
					raster = Raster.createWritableRaster(sampleModel, db, null);
				} else
					throw new UnsupportedOperationException("Unsupported bit depth: " + bitsPerSample);
				return new BufferedImage(cm, raster, false, null);
		...
		switch (e_photoMetric) {
			case PALETTE_COLOR:
				...
				cm = new IndexColorModel(bitsPerSample, rgbColorPalette.length, rgbColorPalette, 0, false, -1,
						DataBuffer.TYPE_BYTE);
				if (bitsPerSample < 8) {
					raster = Raster.createPackedRaster(db, tileWidth * tilesAcross, tileLength * tilesDown, bitsPerSample,
							null);
				} else {
					int[] off = { 0 }; // band offset, we have only one band start at 0
					if (bitsPerSample > 8) {
					...
					raster = Raster.createInterleavedRaster(db, tileWidth * tilesAcross, tileLength * tilesDown,
							tileWidth * tilesAcross, 1, off, null);
				}
				...
				switch (compression) {
					case NONE:
						for (int i = 0; i < tileByteCounts.length; i++) {
							...
							randIS.readFully(temp);
							if (bitsPerSample == 16) {
								raster.setDataElements(xoff, yoff, tileWidth, tileLength,
										ArrayUtils.toShortArray(temp, endian == IOUtils.BIG_ENDIAN));
							} else {
								DataBuffer tileDataBuffer = new DataBufferByte(temp, temp.length);
								WritableRaster tileRaster = Raster.createPackedRaster(tileDataBuffer, tileWidth, tileLength,
										bitsPerSample, null);
				...
				if (decoder != null) {
					for (int i = 0; i < tileByteCounts.length; i++) {
						...
						if (bitsPerSample == 16) {
							raster.setDataElements(xoff, yoff, tileWidth, tileLength,
									ArrayUtils.toShortArray(temp2, endian == IOUtils.BIG_ENDIAN));
						} else
						...
						if (planaryConfiguration == 1) {
							for (int i = 0; i < tileByteCounts.length; i++) {
								...
								if (floatSample) {
									...
									if (bitsPerSample < 8) {
										raster.setDataElements(xoff, yoff, tileWidth, tileLength,
												ArrayUtils.toNBits(bitsPerSample * samplesPerPixel, temp, tileWidth, true));
									} else if (bitsPerSample == 8) {
										raster.setDataElements(xoff, yoff, tileWidth, tileLength, temp);
									} else if (bitsPerSample % 8 == 0) {
										Object tempArray = ArrayUtils.toNBits(bitsPerSample, temp, samplesPerPixel * tileWidth,
												endian == IOUtils.BIG_ENDIAN);
										raster.setDataElements(xoff, yoff, tileWidth, tileLength, tempArray);
									} else {
										Object tempArray = ArrayUtils.toNBits(bitsPerSample, temp, samplesPerPixel * tileWidth,
												true);
							...
							for (int i = 0; i < tilesPerImage; i++) {
								...
								if (floatSample) { // Floating point sample data type
									...
									// Create a TYPE_FLOAT sample model (specifying how the pixels are stored)
									SampleModel sampleModel = new BandedSampleModel(dataBufferType, tileWidth, tileLength,
											tileWidth, bankIndices, bandoff);
									tileRaster = Raster.createWritableRaster(sampleModel, tileDatabuffer, null);
								} else {
									if (bitsPerSample == 16) {
										tileDatabuffer = new DataBufferUShort(
												new short[][] { ArrayUtils.toShortArray(rgb[0], endian == IOUtils.BIG_ENDIAN),
										...
										tileDatabuffer = new DataBufferInt(
												new int[][] {
														(int[]) (ArrayUtils.toNBits(bitsPerSample, rgb[0], tileWidth, bigEndian)),
												...
												tileWidth * tileLength);
									} else if (bitsPerSample == 64) { // Have to convert to 32 bit since there is no
																					// DataBuffer.TYPE_LONG
										tileDatabuffer = new DataBufferInt(
												new int[][] { ArrayUtils.to32BitsLongArray(rgb[0], endian == IOUtils.BIG_ENDIAN),
									...
									tileRaster = Raster.createBandedRaster(tileDatabuffer, tileWidth, tileLength, tileWidth,
											bankIndices, bandoff, null);
								}
	...
	private static byte[] applyDePredictor(int numOfSamples, byte[] input, int imageWidth, int imageHeight) {
		for (int i = 0, inc = numOfSamples * imageWidth, maxVal = inc - numOfSamples,
				minVal = numOfSamples; i <= imageHeight - 1; maxVal += inc, minVal += inc, i++) {
	...
	private static byte[] applyDePredictor2(byte[] input, int offset, int imageWidth, int imageHeight) {
		for (int i = imageHeight - 1, inc = imageWidth, maxVal = offset + inc - 1,
				minVal = offset + 1; i >= 0; maxVal += inc, minVal += inc, i--) {
	...
	private void unpackStrip(byte[] pixels, int offset, int bytes2Read, int stripOffset, int stripByteCount)
			throws IOException {
	...
	private int upsampling(int offsetY, int numOfDataUnit, int bytesPerUnitY, int[] samplingFactor,
			float referenceBlackY, float referenceWhiteY,
			float referenceBlackCb, float referenceWhiteCb, float referenceBlackCr, float referenceWhiteCr,
			float codingRangeY,
