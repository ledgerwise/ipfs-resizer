...
package io.ledgerwise.ipfsresizer.helper.icafe4j.io;
 ...
 * <p>
 * To make it flexible, this class and any of its sub-class doesn't close the
 * underlying
 * stream. It's up to the underlying stream creator to close them. This ensures
 * the actual
 * stream out-lives the random stream itself in case we need to read more
 * content from the
 * underlying stream.
 * <p>
 * NOTE: for MemoryCacheRandomAccessInputStream, there is the risk of "over
 * read" in which
 * more bytes are cached in the buffer than actually needed. In this case, the
 * underlying
