...
package io.ledgerwise.ipfsresizer.helper.icafe4j.image.util;
...
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.quant.NeuQuant;
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.quant.QuantMethod;
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.quant.WuQuant;
...
import io.ledgerwise.ipfsresizer.helper.icafe4j.util.IntHashtable;
...
public class IMGUtils {
	 ...
	 * (i.e.,
	 * a indexed color image), a color map and an index array for the image are also
	 * created.
	 * If the color depth is bigger than 8, only the returned color depth is
	 * relevant.
	 * <p>
	 * This method is not yet completely finished. The colorPalette is currently not
	 * ordered.
	 * A sorting method could be used to find the most used color from the colorFreq
	 * array,
	 * but more tricks will have to be done in order to re-map the newPixels array
	 * to the
	 ...
	 * @param mask      a mask array for transparent pixels - 0 transparent, 1
	 *                  opaque
	 ...
	 * the
	 * images look too bright. Gamma correction usually can compensate for this
	 * problem.
	 * 
			...
			case DataBuffer.TYPE_BYTE:
				...
				if (type == BufferedImage.TYPE_BYTE_INDEXED || type == BufferedImage.TYPE_BYTE_BINARY) {
					...
					if (numOfBanks == 1) {
						...
						for (int i = 0; i < imageSize; i++)
							rgbs[i] = ((bpixels[0][i] & 0xff) << 24) | (bpixels[1][i] & 0xff) | ((bpixels[2][i] & 0xff) << 8)
									| ((bpixels[3][i] & 0xff) << 16);
					}
				} else if (type == BufferedImage.TYPE_3BYTE_BGR) {
					if (numOfBanks == 1) {
	 ...
	 * @param newPixels    a byte array to hold the color map indexes for the image
	 * @param colorPalette the color map for the image
	 * @param fullAlpha    whether or not to keep full alpha or single color
	 *                     transparency
	...
	private static int[] reduceColorsPopularity(int[] rgbTriplets, int colorDepth, byte[] newPixels,
			final int[] colorPalette, boolean fullAlpha) {
			...
			for (int i = no_of_color; i < colorCount; i++) {
				...
				for (int j = 1; j < no_of_color; j++) {
					err2 = (red - clrPalRed[j]) * (red - clrPalRed[j]) + (green - clrPalGreen[j]) * (green - clrPalGreen[j])
							...
							+ (alpha - clrPalAlpha[j]) * (alpha - clrPalAlpha[j]);
					if (err2 < err1) {
						err1 = err2;
						index = j;
					}
		...
		for (int i = 0; i < rgbTriplets.length; i++) {
			...
			boolean trans = (fullAlpha ? (rgbTriplets[i] >>> 24) == 0 || (index & 0xf000) == 0
					: (rgbTriplets[i] >>> 24) < 0x80);
	...
	public static void RGB2CMYK_Inverted(ICC_ColorSpace cmykColorSpace, int[] rgb, float[][] C, float[][] M, float[][] Y,
			float[][] K, int imageWidth, int imageHeight) {
		DataBuffer db = new DataBufferInt(rgb, rgb.length);
		WritableRaster raster = Raster.createPackedRaster(db, imageWidth, imageHeight, imageWidth,
				new int[] { 0x00ff0000, 0x0000ff00, 0x000000ff }, null);
