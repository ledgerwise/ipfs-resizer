...
package io.ledgerwise.ipfsresizer.helper.icafe4j.image.meta.exif;
...
import io.ledgerwise.ipfsresizer.helper.icafe4j.io.MemoryCacheRandomAccessOutputStream;
...
public class ExifThumbnail extends Thumbnail {
	...
	public void write(OutputStream os) throws IOException {
		...
		if (getDataType() == Thumbnail.DATA_TYPE_KJpegRGB) { // Compressed old-style JPEG format
			...
			thumbnailIFD.addField(
					new LongField(TiffTag.JPEG_INTERCHANGE_FORMAT_LENGTH.getValue(), new int[] { compressedImage.length }));
			...
			RandomAccessInputStream tiffIn = new FileCacheRandomAccessInputStream(
					new ByteArrayInputStream(getCompressedImage()));
			...
			thumbnailIFD
					.addField(new ShortField(TiffTag.IMAGE_LENGTH.getValue(), new short[] { (short) thumbnailHeight }));
			thumbnailIFD.addField(new LongField(TiffTag.JPEG_INTERCHANGE_FORMAT.getValue(), new int[] { 0 })); // Place
																																				// holder
			thumbnailIFD.addField(new LongField(TiffTag.JPEG_INTERCHANGE_FORMAT_LENGTH.getValue(), new int[] { 0 })); // Place
																																							// holder
			// Other related tags
			...
			thumbnailIFD.addField(new ShortField(TiffTag.RESOLUTION_UNIT.getValue(), new short[] { 1 })); // No absolute
																																			// unit of
																																			// measurement
			thumbnailIFD.addField(new ShortField(TiffTag.PHOTOMETRIC_INTERPRETATION.getValue(),
					new short[] { (short) TiffFieldEnum.PhotoMetric.YCbCr.getValue() }));
			...
			thumbnailIFD.addField(new ShortField(TiffTag.PLANAR_CONFIGURATTION.getValue(),
					new short[] { (short) TiffFieldEnum.PlanarConfiguration.CONTIGUOUS.getValue() }));
			thumbnailIFD.addField(new ShortField(TiffTag.COMPRESSION.getValue(),
					new short[] { (short) TiffFieldEnum.Compression.OLD_JPG.getValue() }));
			thumbnailIFD
					.addField(new ShortField(TiffTag.ROWS_PER_STRIP.getValue(), new short[] { (short) thumbnailHeight }));
