...
public class TGAReader extends ImageReader {
	...
	private void read_BW_Tga(InputStream is) throws Exception {
		...
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				pix[width * (l + m * i) + n + o * j] = (0xff << 24) | ((brgb[index] & 0xff)) | ((brgb[index] & 0xff) << 8)
						| ((brgb[index++] & 0xff) << 16);
			}
		}
		is.close();
	...
	private void read_RLE_BW_Tga(InputStream is) throws Exception {
		...
		while (p < width * height) {
			...
			if ((brgb[nindex - 1] & 0x80) != 0) {
				for (int q = 0; q < k; q++) {
					pix[width * (l + m * i) + n + o * j] = (0xff << 24) | ((brgb[nindex] & 0xff))
							| ((brgb[nindex] & 0xff) << 8) | ((brgb[nindex] & 0xff) << 16);
					j++;
					if (j % width == 0) {
						i++;
						j = 0;
		...
		if (tgaHeader.bits_per_pixel == 24) { // 24 bit image
			...
			while (p < width * height) {
				...
				if ((brgb[nindex - 1] & 0x80) != 0) {
					for (int q = 0; q < k; q++) {
						pix[width * (l + m * i) + n + o * j] = (0xff << 24) | ((brgb[nindex] & 0xff))
								| ((brgb[nindex + 1] & 0xff) << 8) | ((brgb[nindex + 2] & 0xff) << 16);
						j++;
						if (j % width == 0) {
							i++;
							j = 0;
			...
			while (p < width * height) {
				...
				if ((brgb[nindex - 1] & 0x80) != 0) {
					for (int q = 0; q < k; q++) {
						pix[width * (l + m * i) + n + o * j] = ((brgb[nindex] & 0xff)) | ((brgb[nindex + 1] & 0xff) << 8)
								| ((brgb[nindex + 2] & 0xff) << 16) | (((brgb[nindex + 3] & 0xff) << 24));
						j++;
						if (j % width == 0) {
							i++;
							j = 0;
					...
					for (int q = 0; q < k; q++) {
						pix[width * (l + m * i) + n + o * j] = ((brgb[nindex++] & 0xff)) | ((brgb[nindex++] & 0xff) << 8)
								| ((brgb[nindex++] & 0xff) << 16) | (((brgb[nindex++] & 0xff) << 24));
						j++;
						if (j % width == 0) {
							i++;
							j = 0;
	...
	private void readPalette(InputStream is) throws Exception {
		...
		switch (tgaHeader.colourmap_entry_size) {
			...
			case 24:
				for (int i = 0; i < tgaHeader.colourmap_length - tgaHeader.first_entry_index; i++)
					rgbColorPalette[i] = (0xff << 24) | ((brgb[index++] & 0xff)) | ((brgb[index++] & 0xff) << 8)
							| ((brgb[index++] & 0xff) << 16);
				break;
			case 32:
				for (int i = 0; i < tgaHeader.colourmap_length - tgaHeader.first_entry_index; i++)
					rgbColorPalette[i] = ((brgb[index++] & 0xff)) | ((brgb[index++] & 0xff) << 8)
							| ((brgb[index++] & 0xff) << 16) | ((brgb[index++] & 0xff) << 24);
				break;
			default:
		}
	}
