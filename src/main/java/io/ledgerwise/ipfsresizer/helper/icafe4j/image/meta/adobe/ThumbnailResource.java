...
public class ThumbnailResource extends _8BIM {
	...
	// id is either ImageResourceID.THUMBNAIL_RESOURCE_PS4 or
	// ImageResourceID.THUMBNAIL_RESOURCE_PS5
	...
	private void setThumbnailImage(ImageResourceID id, int dataType, int width, int height, int totalSize,
			byte[] thumbnailData) {
		// JFIF data in RGB format. For resource ID 1033 (0x0409) the data is in BGR
		// format.
		if (dataType == Thumbnail.DATA_TYPE_KJpegRGB) {
			...
			WritableRaster raster = Raster.createInterleavedRaster(db, width, height, paddedRowBytes, numOfBands, off,
					null);
