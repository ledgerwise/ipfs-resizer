...
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.png.Chunk;
...
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.png.IENDBuilder;
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.png.IHDRBuilder;
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.png.PLTEBuilder;
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.png.PNGTweaker;
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.png.TIMEBuilder;
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.png.TRNSBuilder;
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.png.TextBuilder;
...
public class PNGWriter extends ImageWriter {
	...
	// Apply dynamic filtering by "the minimum sum of absolute differences" (MSAD)
	// heuristic
	private static void apply_adaptive_filter(int[] filter_type, byte[] pixBytes, int height, int bytesPerPixel,
	...
	private static void apply_filter(int[] filter_type, byte[] pixBytes, int height, int bytesPerPixel,
			int bytesPerScanLine) {
		//
		for (int j = height - 1, offset = pixBytes.length - bytesPerScanLine; j >= 0; j--, offset -= bytesPerScanLine) {
	...
	protected void write(int[] pixels, int imageWidth, int imageHeight, OutputStream os) throws Exception {
		...
		/*
		 * We could have put all the chunks including IDAT and IEND into the chunks list
		 * and serialize them in the
		 * end. But this is memory consuming. To save memory, we don't put IDAT chunks
		 * into chunks list but write
		 * them right before IEND.
		 */
		new IENDBuilder().build().write(os);
	}

	private void writeGrayScale(byte[] pixels, int imageWidth, int imageHeight, boolean hasAlpha, OutputStream os)
			throws Exception {
		// The rule of thumb is don't use any filter for gray-scale image but in some
		// cases, PAETH or adaptive filter does much better
		// Add IHDR chunk
		...
		// PNG allows for 8 and 16 bits for gray-scale with alpha, for now, only 8 bit
		// is supported
		int bitsPerPixel = 8;
		...
		if (!hasAlpha && imageParam.isTransparent()) {
			...
			byte trans_color = (byte) (((transparentColor >> 16) & 0xff) * 0.2126
					+ ((transparentColor >> 8) & 0xff) * 0.7152 + (transparentColor & 0xff) * 0.0722);
		...
		// This should be called for the last chunk to make sure we get all the input
		// data compressed
	...
	private void writeRGB(int[] pixels, int imageWidth, int imageHeight, OutputStream os) throws Exception {
		// The rule of thumb is always use PAETH filter which, in most cases, is as good
		// as adaptive filter and much faster
		boolean noAlpha = !imageParam.hasAlpha();
		// Add IHDR chunk
		IHDRBuilder hdrBuilder = new IHDRBuilder().width(imageWidth).height(imageHeight).bitDepth(8).compressionMethod(0)
				.filterMethod(0).interlaceMethod(0);
		...
		if (noAlpha && imageParam.isTransparent()) {
			...
			byte[] alpha = new byte[] { 0, (byte) (transparentColor >>> 16), 0, (byte) (transparentColor >>> 8), 0,
		...
		if (isApplyAdaptiveFilter) {
			// PNG specification suggests adaptive filter for RGB or grayscale image with 8
			// or more bit depth
