...
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.compression.huffman.HuffmanEncoder;
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.jpeg.HTable;
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.jpeg.JPGConsts;
...
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.util.DCT;
...
public class JPGWriter extends ImageWriter {
	...
	// TODO: may need more changes to work with CMYK or RGB, and perhaps YCCK color
	// space
	...
	private void RGB2RGB(int[] rgb, float[][] r, float[][] g, float[][] b, int imageWidth, int imageHeight)
			throws Exception {
	...
	protected void write(int[] pixels, int imageWidth, int imageHeight, OutputStream os) throws Exception {
		...
		// Write JPEG tables if needed (for TIFF Technote2, JPEG tables can be written
		// separately)
		if (includeTables) {
			// Write DQT
			writeDQT(os);
	...
	private void writeGrayScale(float[][] pixels, OutputStream os, int imageWidth, int imageHeight) throws Exception {
		...
		// If we are going to use custom encoder tables, call
		// encoder.setEncodingTables() here before
		// calling encoder.initialize() which will skip the default encoding tables
		// generation
		encoder.initialize();
	...
	private void writeFullColor(int[] pixels, OutputStream os, int imageWidth, int imageHeight) throws Exception {
		...
		// If we are going to use custom encoder tables, call
		// encoder.setEncodingTables() here before
		// calling encoder.prepare() which will skip the default encoding tables
		// generation
		encoder.initialize();
	...
	/**
	 * Write default JPEG tables for TIFF image only. The following are allowed
	 * markers.
