...
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.util.BytePacker;
...
public class PCXReader extends ImageReader {
				...
				switch (bitsPerPixel) {
					...
					case 3:
						throw new UnsupportedOperationException(
								"Invalid bitsPerPixel: " + bitsPerPixel + " for one plane PCX image");
					case 2:
	...
	private void readPalette(byte[] buf) throws Exception {
		...
		for (int i = 0; i < buf.length; i += 3) {
			rgbColorPalette[index++] = ((0xff << 24) | ((buf[nindex++] & 0xff) << 16) | ((buf[nindex++] & 0xff) << 8)
					| (buf[nindex++] & 0xff));
		}
	}
	...
	private BufferedImage read256ColorPcx(InputStream is) throws Exception {
		 ...
		 * the beginning of the color palette, fill the color palette and reset the
		 * pointer to
		 * the beginning of the image data. This is awkward and equally memory consuming
		 * since
		 * the system must maintain a buffer for the reset() method and we have to keep
		 * track
		 * of the skip(long) method to ensure reading of the required bytes as shown
		 * below:
		 * 
		 * <p>
		 * is.mark(available);
		...
		ColorModel cm = new IndexColorModel(8, rgbColorPalette.length, rgbColorPalette, 0, false, -1,
	...
	private BufferedImage readOneBitEgaPcx(InputStream is) throws Exception {
		...
		image: for (int i = 0; i < height; i++, index = 0) {
			do {
				bt = brgb[nindex++] & 0xff;
				if ((bt & 0xC0) == 0xC0) {
					num_of_rep = bt & 0x3F;
			...
			scanLine: for (int k = 0; k < bytesPerLine; k++) {
				for (int l = 7; l >= 0; l--) {
	...
	private static class PcxHeader {
		...
		void readHeader(InputStream is) throws Exception {
			...
			for (int i = 0; i < 16; i++) {
				colorPalette[i] = ((0xff << 24) | ((buf[nindex++] & 0xff) << 16) | ((buf[nindex++] & 0xff) << 8)
						| (buf[nindex++] & 0xff));
			}
