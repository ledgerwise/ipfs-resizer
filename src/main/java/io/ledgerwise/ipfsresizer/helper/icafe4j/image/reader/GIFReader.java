...
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.gif.GIFFrame;
...
public class GIFReader extends ImageReader {
	...
	// BufferedImage with the width and height of the logical screen to draw frames
	// upon
	 ...
	 * logical screen and may also rely on transparency and previous frames to work
	 * properly.
	 * <p>
	 ...
	 * @return a BufferedImage for the image or current frame in case of animated
	 *         GIF
	 * @throws Exception
	 */
	protected BufferedImage getFrameAsBufferedImage(InputStream is) throws Exception {
		...
		ColorModel cm = new IndexColorModel(bitsPerPixel, rgbColorPalette.length, rgbColorPalette, 0, false,
				transparent_color, DataBuffer.TYPE_BYTE);

		return new BufferedImage(cm, raster, false, null);
	}

	/**
	 * Gets the current frame as a BufferedImage the same size as the logical
	 * screen.
	 * Graphic Control Extension and Image Descriptor parameters are taken into
	 * account
	 * When creating the frame. The resulting frame is actually a "composite" one or
	 * a
	 * snapshot as seen in an animated GIF. Such frames may not be the same as the
	 * frames
	 * created by {@link #getFrameAsBufferedImage(InputStream)
	 * getFrameAsBufferedImage}
	 * which could be of different sizes and may also have to rely on the previous
	 * frames
	 ...
	 * Use them separately.
	 * 
	 * @param is input stream for the image - single frame or multiple frame
	 *           animated GIF
	...
	protected BufferedImage getFrameAsBufferedImageEx(InputStream is) throws Exception {
		...
		// Create a backup bufferedImage from the base image for the area of the current
		// frame
		...
		// We need to clone the base image since we are going to dispose it later
	 ...
	 * 
	 * @return a list of the total frames read by this GIFRreader or empty list if
	 *         not read yet
	 */
	...
	private byte[] readFrame(InputStream is) throws Exception {
		...
		do {
			...
			if (image_separator == 0x21) // (!) Extension Block
			{
				...
				if (func == 0xf9) { // Graphic Control Label - identifies the current block as a Graphic Control
											// Extension
					int packedFields = is.read();
					// Determine the disposal method
					disposalMethod = ((packedFields & 0x1c) >> 2);
					LOGGER.info("Disposal method: {}",
							(disposalMethod == 0) ? "UNSPECIFIED"
									: (disposalMethod == 1) ? "LEAVE_AS_IS"
											: (disposalMethod == 2) ? "RESTORE_TO_BACKGROUND"
		...
		if (!hasLocalColorMap)
			rgbColorPalette = globalColorPalette;
	...
	private void readGlobalPalette(InputStream is, int num_of_color) throws Exception {
		...
		for (int i = 0; i < num_of_color; i++)
			globalColorPalette[i] = ((255 << 24) | ((brgb[index1++] & 0xff) << 16) | ((brgb[index1++] & 0xff) << 8)
					| (brgb[index1++] & 0xff));
	}

	private boolean readGlobalScopeData(InputStream is) throws Exception {
		// Global scope data including header, logical screen descriptor, global
		// colorPalette if presents
	...
	public BufferedImage read(InputStream is) throws Exception {
		...
		while ((bi = getFrameAsBufferedImageEx(is)) != null) {
			gifFrames.add(new GIFFrame(bi, image_x, image_y, delay, disposalMethod, userInputFlag, transparencyFlag,
					transparent_color));
			frames.add(bi);
		}
	...
	private void readLocalPalette(InputStream is, int num_of_color) throws Exception {
		...
		for (int i = 0; i < num_of_color; i++)
			rgbColorPalette[i] = ((255 << 24) | ((brgb[index1++] & 0xff) << 16) | ((brgb[index1++] & 0xff) << 8)
					| (brgb[index1++] & 0xff));
	}
