...
package io.ledgerwise.ipfsresizer.helper.icafe4j.image.meta.adobe;
...
public enum ImageResourceID {
	...
	PRINT_FLAGS(
			"Print flags. A series of one byte boolean values: labels, crop marks, color bars, registration marks, negative, flip, interpolate, caption.",
			(short) 0x03f3),
	...
	LAYER_STATE_INFO("Layer state information. 2 bytes containing the index of target layer. 0=bottom layer.",
			(short) 0x0400),
	...
	COPYRIGHT_FLAG(
			...
			(short) 0x040a),
	URL("URL. Handle of a text string with uniform resource locator. Can be set via Property suite or by user in File Info...",
			(short) 0x040b),
	...
	GLOBLE_ANGLE(
			...
			(short) 0x040d),
	COLOR_SAMPLERS_RESOURCE("Color samplers resource. See color samplers resource format later in this chapter.",
			(short) 0x040e),
	...
	EFFECTS_VISIBLE(
			...
			(short) 0x0412),
	SPOT_HALFTONE("Spot Halftone. 4 bytes for version, 4 bytes for length, and the variable length data.",
			(short) 0x0413),
	DOC_SPECIFIC_ID(
			"Document specific IDs, layer IDs will be generated starting at this base value or a greater value if we find existing IDs to already exceed it. 4 bytes.",
			(short) 0x0414),
	...
	ALPHA_IDENTIFIERS("Alpha Identifiers. 4 bytes of length, followed by 4 bytes each for every alpha identifier.",
			(short) 0x041d),
	URL_LIST("URL List. 4 byte count of URLs, followed by 4 byte long, 4 byte ID, and unicode string for each count.",
			(short) 0x041e),
	VERSION_INFO(
			"(Photoshop 6.0) Version Info. 4 byte version, 1 byte HasRealMergedData, unicode string of writer name, unicode string of reader name, 4 bytes of file version.",
			(short) 0x0421),
	...
	CAPTION_DIGEST("(Photoshop 7.0) Caption digest. 16 bytes: RSA Data Security, MD5 message-digest algorithm.",
			(short) 0x0425),
	PRINT_SCALE(
	...
	ALTERNATE_DUOTONE_COLORS(
			"(Photoshop CS) Alternate Duotone Colors.  bytes (version = 1), 2 bytes count, following is repeated for each count: [ Color: 2 bytes for space followed by 4 * 2 byte color component ], following this is another 2 byte count, usually 256, followed by Lab colors one byte each for L, a, b.",
			(short) 0x042a),
	ALTERNATE_SPOT_COLORS(
			"(Photoshop CS)Alternate Spot Colors. 2 bytes (version = 1), 2 bytes channel count, following is repeated for each count: 4 bytes channel ID, Color: 2 bytes for space followed by 4 * 2 byte color component",
			(short) 0x042b),
	LAYER_SELECTION_IDS(
	...
	LAYER_GROUP_ENABLED_ID(
			...
			(short) 0x0430),
	COLOR_SAMPLERS_RESOURCE_CS3("(Photoshop CS3) Color samplers resource. Also see ID 1038 for old format.",
			(short) 0x0431),
	MEASUREMENT_SCALE("(Photoshop CS3) Measurement Scale. 4 bytes (descriptor version = 16), Descriptor.",
			(short) 0x0432),
	...
	DISPLAY_INFO_STRUCTURE("(Photoshop CS3) DisplayInfo structure to support floating point clors. Also see ID 1007.",
			(short) 0x0435),
