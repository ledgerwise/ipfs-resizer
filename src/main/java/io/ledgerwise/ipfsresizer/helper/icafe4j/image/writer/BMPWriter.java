...
public class BMPWriter extends ImageWriter {
	...
	protected void write(int[] pixels, int imageWidth, int imageHeight,
		...
		if (getImageParam().getColorType() == ImageColorType.INDEXED)
			write256ColorBitmap(pixels, imageWidth, imageHeight, os);
		else
			writeTrueColorBitmap(pixels, imageWidth, imageHeight, os);
	}
	...
	private void write256ColorBitmap(int[] pixels, int imageWidth,
		...
		if (param.isApplyDither()) {
			if (param.getDitherMethod() == DitherMethod.FLOYD_STEINBERG)
				IMGUtils.reduceColorsDiffusionDither(param.getQuantMethod(), pixels, imageWidth, imageHeight, 8, newPixels,
						colorPalette);
			else
				IMGUtils.reduceColorsOrderedDither(param.getQuantMethod(), pixels, imageWidth, imageHeight, 8, newPixels,
						colorPalette, param.getDitherMatrix());
		} else
