...
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.meta.MetadataEntry;
...
public abstract class Exif extends Metadata {
	...
	public Iterator<MetadataEntry> iterator() {
		...
		if (containsThumbnail) {
			...
			thumbnailEntry.addEntry(new MetadataEntry("Thumbnail format",
					(thumbnail.getDataType() == 1 ? "DATA_TYPE_KJpegRGB" : "DATA_TYPE_TIFF")));
			thumbnailEntry
					.addEntry(new MetadataEntry("Thumbnail data length", "" + thumbnail.getCompressedImage().length));
			...
			if (ifds.size() >= 2) {
				...
				if (field != null) { // JPEG format, save as JPEG
					...
					if (thumbnailLen > 0) {
						...
						thumbnail = new ExifThumbnail(width, height, Thumbnail.DATA_TYPE_KJpegRGB, thumbnailData,
								thumbnailIFD);
					...
					if (field != null) {
						...
						thumbnail = new ExifThumbnail(width, height, Thumbnail.DATA_TYPE_TIFF, bout.toByteArray(),
								thumbnailIFD);
						containsThumbnail = true;
					}
				}
