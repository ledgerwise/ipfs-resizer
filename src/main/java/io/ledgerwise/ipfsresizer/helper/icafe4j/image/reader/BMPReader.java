...
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.bmp.BmpCompression;
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.options.BMPOptions;
...
public class BMPReader extends ImageReader {
	...
	private void readPalette(InputStream is) throws Exception {
		...
		for (int i = 0; i < numOfColors; i++) {
			rgbColorPalette[index++] = ((0xff << 24) | (brgb[nindex] & 0xff) | ((brgb[nindex + 1] & 0xff) << 8)
	...
	private BufferedImage read24bitTrueColorBitmap(InputStream is) throws Exception {
		...
		ColorModel cm = new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB), false, false, trans,
				DataBuffer.TYPE_BYTE);

		return new BufferedImage(cm, raster, false, null);
	}

	// This actually deals with the case of RGB888 mask case for the 32 bits image
	// but it seems to work for other mask too
		...
		if (alignment == BMPOptions.ALIGN_BOTTOM_UP) {
			for (int i = 1, index = 0; i <= height; i++) {
				...
				for (int j = 0, nindex = 0; j < width; j++) {
					pix[index++] = ((brgb[nindex++] & 0xff) | ((brgb[nindex++] & 0xff) << 8)
							| ((brgb[nindex++] & 0xff) << 16) | (0xff << 24));
					nindex++;
				}
			}
		} else {
	...
	private BufferedImage readCompressedIndexColorBitmap(InputStream is) throws Exception {
		...
		ColorModel cm = new IndexColorModel(bitsPerPixel, rgbColorPalette.length, rgbColorPalette, 0, false, -1,
				DataBuffer.TYPE_BYTE);
	...
	private static class BitmapHeader {
		 ...
		 * imageSize = int(((imageWidth * planes * bitCount) + 31) / 32) * 4 *
		 * imageHeight
		...
		void readHeader(InputStream is) throws Exception {
			...
			fileSize = (bhdr[nindex++] & 0xff) | ((bhdr[nindex++] & 0xff) << 8) | ((bhdr[nindex++] & 0xff) << 16)
					| ((bhdr[nindex++] & 0xff) << 24);
			reserved1 = (short) ((bhdr[nindex++] & 0xff) | ((bhdr[nindex++] & 0xff) << 8));
			reserved2 = (short) ((bhdr[nindex++] & 0xff) | ((bhdr[nindex++] & 0xff) << 8));
			dataOffSet = (bhdr[nindex++] & 0xff) | ((bhdr[nindex++] & 0xff) << 8) | ((bhdr[nindex++] & 0xff) << 16)
					| ((bhdr[nindex++] & 0xff) << 24);
			infoHeaderLen = (bhdr[nindex++] & 0xff) | ((bhdr[nindex++] & 0xff) << 8) | ((bhdr[nindex++] & 0xff) << 16)
					| ((bhdr[nindex++] & 0xff) << 24);
			...
			nindex = 0;
			imageWidth = (bhdr[nindex++] & 0xff) | ((bhdr[nindex++] & 0xff) << 8) | ((bhdr[nindex++] & 0xff) << 16)
					| ((bhdr[nindex++] & 0xff) << 24);
			imageHeight = (bhdr[nindex++] & 0xff) | ((bhdr[nindex++] & 0xff) << 8) | ((bhdr[nindex++] & 0xff) << 16)
					| ((bhdr[nindex++] & 0xff) << 24);
			planes = (short) ((bhdr[nindex++] & 0xff) | ((bhdr[nindex++] & 0xff) << 8));
			bitCount = (short) ((bhdr[nindex++] & 0xff) | ((bhdr[nindex++] & 0xff) << 8));
			compression = (bhdr[nindex++] & 0xff) | ((bhdr[nindex++] & 0xff) << 8) | ((bhdr[nindex++] & 0xff) << 16)
					| ((bhdr[nindex++] & 0xff) << 24);
			imageSize = (bhdr[nindex++] & 0xff) | ((bhdr[nindex++] & 0xff) << 8) | ((bhdr[nindex++] & 0xff) << 16)
					| ((bhdr[nindex++] & 0xff) << 24);
			xResolution = (bhdr[nindex++] & 0xff) | ((bhdr[nindex++] & 0xff) << 8) | ((bhdr[nindex++] & 0xff) << 16)
					| ((bhdr[nindex++] & 0xff) << 24);
			yResolution = (bhdr[nindex++] & 0xff) | ((bhdr[nindex++] & 0xff) << 8) | ((bhdr[nindex++] & 0xff) << 16)
					| ((bhdr[nindex++] & 0xff) << 24);
			colorsUsed = (bhdr[nindex++] & 0xff) | ((bhdr[nindex++] & 0xff) << 8) | ((bhdr[nindex++] & 0xff) << 16)
					| ((bhdr[nindex++] & 0xff) << 24);
			colorsImportant = (bhdr[nindex++] & 0xff) | ((bhdr[nindex++] & 0xff) << 8) | ((bhdr[nindex++] & 0xff) << 16)
					| ((bhdr[nindex++] & 0xff) << 24);
		}
	}
}
