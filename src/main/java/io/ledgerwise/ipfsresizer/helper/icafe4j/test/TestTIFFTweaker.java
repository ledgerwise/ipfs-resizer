...
import io.ledgerwise.ipfsresizer.helper.icafe4j.util.FileUtils;
	...
	public void test(String... args) throws Exception {
		...
		if (args.length > 1) {
			if (args[1].equalsIgnoreCase("copycat")) {
				...
				ImageParam param = builder.colorType(ImageColorType.BILEVEL).applyDither(true)
						.ditherMethod(DitherMethod.BAYER).imageOptions(tiffOptions).build();
				...
				if (args[1].equalsIgnoreCase("writemultipage")) {
					...
					// Put the pixels on the raster. Note that only values 0 and 1 are used for the
					// pixels.
