...
public class G32DEncoder extends G31DEncoder implements ImageEncoder {
	...
	// Need to take care of processing the first and last picture element in a line:
	// ITU-T Rec. T4 (07/2003) 4.2.1.3.4
		...
		while (!endOfLine) {
			...
			if (offset >= scanLineWidth) {
				...
				if (a0 == -1)
					...
					if (bias == 7)
						color = ((pixels[begin - 1]) & 0x01);
					else
						color = ((pixels[begin] >>> (bias + 1)) & 0x01);
				}
			...
			if (begin < 0) {
				...
				if (b1 == -1)
					b1 = scanLineWidth; // Situated just after the last changing element on the reference line
			}
			// Determine b2
			color ^= 1;
			offset = b1 + 1;
			...
			if (offset >= scanLineWidth) {
				...
				if (b2 == -1)
					b2 = scanLineWidth; // Situated just after the last changing element on the reference line
			}
			...
			if (b2 < a1) {// Yes, we are in pass code
				...
				if (Math.abs(a1b1) <= 3) { // We are in Vertical mode
					...
					int len = a1 - a0;
					if (a0 == -1)
						len--; // First changing element of the scan line
	...
	private int findChangingElement(byte[] pixels, int begin, int color, int bias, int offset, boolean sameLine) {
		...
		int upperColor = ((a0 == -1) ? 0
				: ((bias == 7) ? (pixels[begin - 1] & 0x01) : ((pixels[begin] >>> (bias + 1)) & 0x01)));
