...
package io.ledgerwise.ipfsresizer.helper.icafe4j.image.meta.icc;
...
public enum ProfileTag {
	...
	bXYZ(TagType.PUBLIC, 0x6258595A, "blueMatrixColumnTag"), // The third column in the matrix used in matrix/TRC
																				// transforms. (This column is combined with the linear blue
	...
	chad(TagType.PUBLIC, 0x63686164, "chromaticAdaptationTag"), // Converts an nCIEXYZ colour relative to the actual
																					// adopted white to the nCIEXYZ colour relative to the
																					// PCS adopted white. Required only if the chromaticity
																					// of the actual adopted white is different from that of
	...
	clrt(TagType.PUBLIC, 0x636C7274, "colorantTableTag"), // Identifies the colorants used in the profile. Required for
																			// N-component based Output profiles and DeviceLink profiles
																			// only if the data colour space field is xCLR (e.g. 3CLR)
	clot(TagType.PUBLIC, 0x636C6F74, "colorantTableOutTag"), // Identifies the output colorants used in the profile,
																				// required only if the PCS Field is xCLR (e.g. 3CLR)
	ciis(TagType.PUBLIC, 0x63696973, "colorimetricIntentImageStateTag"), // Indicates the image state of PCS colorimetry
																								// produced using the colorimetric intent
																								// transforms
	cprt(TagType.PUBLIC, 0x63707274, "copyrightTag"), // Profile copyright information
	desc(TagType.PUBLIC, 0x64657363, "profileDescriptionTag"), // Structure containing invariant and localizable versions
																					// of the profile name for displays
	...
	gXYZ(TagType.PUBLIC, 0x6758595A, "greenMatrixColumnTag"), // The second column in the matrix used in matrix/TRC
																					// transforms (This column is combined with the linear
																					// green channel during the matrix multiplication).
	...
	rigo(TagType.PUBLIC, 0x72696730, "perceptualRenderingIntentGamutTag"), // When present, the specified gamut is
																									// defined to be the reference medium gamut
																									// for the PCS side of both the A2B0 and B2A0
																									// tags
	rig2(TagType.PUBLIC, 0x72696732, "saturationRenderingIntentGamutTag"), // When present, the specified gamut is
																									// defined to be the reference medium gamut
																									// for the PCS side of both the A2B2 and B2A2
																									// tags
	rXYZ(TagType.PUBLIC, 0x7258595A, "redMatrixColumnTag"), // The first column in the matrix used in matrix/TRC
																				// transforms. (This column is combined with the linear red
																				// channel during the matrix multiplication).
	...
	tech(TagType.PUBLIC, 0x74656368, "technologyTag"), // Device technology information such as LCD, CRT, Dye
																		// Sublimation, etc.
