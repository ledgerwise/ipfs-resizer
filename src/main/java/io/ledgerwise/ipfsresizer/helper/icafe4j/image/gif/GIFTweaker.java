...
package io.ledgerwise.ipfsresizer.helper.icafe4j.image.gif;
...
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.meta.gif.GifXMP;
...
import io.ledgerwise.ipfsresizer.helper.icafe4j.image.writer.GIFWriter;
...
public class GIFTweaker {
	 ...
	 *                            or equal zero, it will be determined from the
	 *                            first frame
	 ...
	 * @throws Exception
	 */
	public static void prepareForWrite(GIFWriter writer, OutputStream os, int logicalScreenWidth,
			int logicalScreenHeight) throws Exception {
				...
				if (func == 0xf9) {
					...
					switch (disposalMethod) {
						...
						case GIFFrame.DISPOSAL_RESTORE_TO_PREVIOUS:
							...
							break;
						default:
							// throw new RuntimeException("Invalid GIF frame disposal method: " +
							// disposalMethod);
					}
	...
	public static void splitFrames(InputStream is, String outputFilePrefix) throws Exception {
		...
		do {
			outFileName = StringUtils.isNullOrEmpty(outputFilePrefix) ? "frame_" + frameCount++
					: outputFilePrefix + "_frame_" + frameCount++;
	 ...
	 * @throws Exception
	 * 
	 * @deprecated Use {@link #splitAnimatedGIF(InputStream, ImageWriter, String)
	 *             splitAnimagedGIF} instead.
	 */
	@Deprecated
	public static void splitFrames2(InputStream animatedGIF, ImageWriter writer, String outputFilePrefix)
			throws Exception {
		...
		// After reading the global scope data, we can retrieve values such logical
		// screen width and height etc.
		int logicalScreenWidth = reader.getLogicalScreenWidth();
		int logicalScreenHeight = reader.getLogicalScreenHeight();
		// Then create a BufferedImage with the width and height of the logical screen
		// and draw frames upon it
	 ...
	 * @throws Exception
	 */
	public static void splitAnimatedGIF(InputStream animatedGIF, ImageWriter writer, String outputFilePrefix)
			throws Exception {
		// Create a GIFReader to read GIF frames
		FrameReader reader = new FrameReader();
		// Create a GIFWriter or other writers to write the frames
	 ...
	 */
	public static void writeAnimatedGIF(BufferedImage[] images, int[] delays, int loopCount, OutputStream os)
			throws Exception {
	 ...
	 * @param os        OutputStream to write the image
	 */
	public static void writeAnimatedGIF(BufferedImage[] images, int[] delays, int loopCount, ImageParam param,
	...
	/**
	 * Write an array of GIFFrame as an animated GIF. This method gives a user more
	 ...
	 * @throws Exception
	 */
	public static void writeAnimatedGIF(GIFFrame[] frames, int loopCount, ImageParam param, OutputStream os)
			throws Exception {
		GIFWriter writer = new GIFWriter();
		writer.setImageParam(param);
		writer.setLoopCount(loopCount);
